...
import com.mygdx.BotAlgorithms.Stupid;
...
public class GameScreenAuto extends InputAdapter implements Screen {
    ...
    public GameScreenAuto(CrazyPuttingGame game, int level, int alg) {
        this.score = 0;
        this.game = game;
        this.alg = alg;
    ...
    public void render(float delta) {
        if (loaded && ready) {
            ...
            if (rollingBall.isStationary() && state == 1) {
                ...
                if (m.getEndPos().dst(tmpPos.x, tmpPos.y) < 1) {
                    state = 2;
                    game.showWinScreenAuto(this.score, this.level);
                }
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.SPACE) && this.state == 0) {
            ...
            System.out.println(prove + " " + proveAngle);
            this.state = 1;
            this.score++;*/
            ...
            if (this.alg == 1) {
                ...
                ballPosition.x = rollingBall.getPosition().x;
                ballPosition.y = rollingBall.getPosition().y;
                int[] shootingInfo = bot.getNextShot(ballPosition);
                rollingBall.setVelocity(shootingInfo[1] / 10, fromDegreeToRadians(shootingInfo[0]));
                this.state = 1;
                this.score++;

            } else if (this.alg == 2) {

                //the bot number 2 should play the game

            } else {

                //the bot number 3 should play the game
